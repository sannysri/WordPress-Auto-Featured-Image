...
jQuery(document).ready(function($) {
	...
	var wpafi = {
		toast: function(message, type) {
			...
			var $toast = $('<div class="wpafi-toast ' + type + '">' +
				'<span class="dashicons ' + icons[type] + ' wpafi-toast-icon"></span>' +
				'<span class="wpafi-toast-message">' + message + '</span>' +
	...
	$('#upload_default_thumb').on('click', function(e) {
		...
		mediaUploader.on('select', function() {
			...
			$('#uploaded_thumb_preview').html('<img src="' + attachment.url + '" alt="Default featured image">');
	...
	(function() {
		...
		var $allNotices = $('#wpafi-settings-messages .notice, .wpafi-settings-wrap .notice, .wpafi-settings-wrap .updated, .wpafi-settings-wrap .error');
		
		$allNotices.each(function() {
			var $notice = $(this);
			var message = $notice.find('p').text() || $notice.text();
			var type = 'success';
	...
	function runBulkOperation() {
		var $button = $('#wpafi-bulk-assign');
		var $spinner = $('#wpafi-bulk-spinner');
		var $result = $('#wpafi-bulk-result');
		var $progressContainer = $('#wpafi-progress-container');
		...
		var progressInterval = setInterval(function() {
			var current = parseInt($('#wpafi-progress-fill').css('width')) / $('#wpafi-progress-bar').width() * 100;
			if (current < 90) {
				wpafi.updateProgress(Math.min(current + 10, 90), 'Processing...');
			}
		}, 500);
	...
	$('.wpafi-help-tip').each(function() {
		...
		$tip.on('mouseenter', function() {
			var $tooltip = $('<div class="wpafi-tooltip">' + title + '</div>');
			$('body').append($tooltip);

			var tipOffset = $tip.offset();
			$tooltip.css({
				top: tipOffset.top - $tooltip.outerHeight() - 8,
				left: tipOffset.left - ($tooltip.outerWidth() / 2) + ($tip.outerWidth() / 2)
			});
		});
	...
	var maxRules = wpafi_vars.max_rules || 2;
	var ruleIndex = $('#wpafi-rules-container .wpafi-rule-card').length;
	...
	$('#wpafi-add-rule').on('click', function(e) {
		...
		var currentRuleCount = $('#wpafi-rules-container .wpafi-rule-card').length;

		if (currentRuleCount >= maxRules) {
			wpafi.toast(wpafi_vars.max_rules_message || 'Upgrade to Pro for unlimited rules!', 'warning');
			return;
		}

		var template = $('#wpafi-rule-template').html();
		template = template.replace(/\{\{INDEX\}\}/g, ruleIndex);

		$('#wpafi-rules-container').append(template);
		
		// Update rule number display.
		$('#wpafi-rules-container .wpafi-rule-card').last().find('.wpafi-rule-index').text(ruleIndex + 1);
	...
	function updateRuleNumbers() {
		$('#wpafi-rules-container .wpafi-rule-card').each(function(i) {
			$(this).find('.wpafi-rule-index').text(i + 1);
		});
	}
	...
	function updateRuleLimitText() {
		var currentRuleCount = $('#wpafi-rules-container .wpafi-rule-card').length;
		var $container = $('.wpafi-add-rule-container');
		var $limitElement = $container.find('.wpafi-rule-limit');
		...
		if (currentRuleCount >= maxRules) {
			...
			if (!$limitElement.hasClass('wpafi-rule-limit-locked')) {
				...
				$limitElement.html(
					...
					'<span>' + currentRuleCount + ' of ' + maxRules + ' rules used</span>' +
			...
			$limitElement.html(currentRuleCount + ' of ' + maxRules + ' rules used');
	...
	$(document).on('click', '.wpafi-select-image, .wpafi-rule-image-preview', function(e) {
		e.preventDefault();

		var $element = $(this);
		var $card = $element.closest('.wpafi-rule-card');
		var $imageId = $card.find('.wpafi-rule-image-id');
		var $preview = $card.find('.wpafi-rule-image-preview');
		var $removeBtn = $card.find('.wpafi-remove-image');
		...
		mediaUploader.on('select', function() {
			...
			$preview.html('<img src="' + attachment.url + '" alt="Featured image">');
			$removeBtn.show();
	...
	$(document).on('click', '.wpafi-remove-image', function(e) {
		...
		var $card = $(this).closest('.wpafi-rule-card');
		var $imageId = $card.find('.wpafi-rule-image-id');
		var $preview = $card.find('.wpafi-rule-image-preview');
	...
	$(document).on('change', '.wpafi-image-source-options input[type="radio"]', function() {
		var $card = $(this).closest('.wpafi-rule-card');
		var source = $(this).val();
	...
	function updateBulkRuleDropdown() {
		...
		$('#wpafi-rules-container .wpafi-rule-card').each(function(i) {
			var $card = $(this);
			var ruleName = $card.find('.wpafi-rule-name').val();
			var displayName = ruleName || 'Rule #' + (i + 1);
			$dropdown.append('<option value="' + i + '">' + displayName + '</option>');
		});
	...
	var originalAddRule = $('#wpafi-add-rule').data('events');
	...
	$('#wpafi-settings-form').on('submit', function(e) {
		...
		$('#wpafi-rules-container .wpafi-rule-card').each(function(index) {
			var $card = $(this);
			var imageId = $card.find('.wpafi-rule-image-id').val();
			var ruleName = $card.find('.wpafi-rule-name').val() || 'Rule #' + (index + 1);
		...
		if (hasErrors) {
			...
			var $firstError = $('.wpafi-rule-error').first();
